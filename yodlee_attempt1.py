...
def loadAccounts():
    ...
    INFO['accounts'] = req.json()['account']
...
def getTransactions(accountId, from_year=1900, from_month=1, from_day=1, to_year=2020, to_month=1, to_day=1, container=None):
    data = {
        "accountId": accountId,
...
def getTransactionsSummary(account, category_type, from_year=2000, from_month=1, from_day=1, to_year=2020, to_month=1, to_day=1):
    data = {
        'accountId': account,
...
def getTransactionsCount(account, from_year=1900, from_month=1, from_day=1, to_year=2020, to_month=1, to_day=1, container=None):
    data = {
        "accountId": account,
        "fromDate": "%d-%02d-%02d" % (from_year, from_month, from_day),
        "toDate": "%d-%02d-%02d" % (to_year, to_month, to_day)
...
if __name__ == '__main__':
    ...
    loadAccounts()
    # print(json.dumps(getTransactionsSummary(INFO['accounts'][0]['id'], 'EXPENSE'), indent=4))
    for acc in INFO['accounts']:
        print("Summary for %s:" % acc['accountName'])
        try:
            expenseData = getTransactionsSummary(acc['id'], 'EXPENSE')['transactionSummary']
        except:
            print("Expenses: None")
        else:
            print("Expenses:")
            expenseByCategory = expenseData[0]['categorySummary']
            for c in expenseByCategory:
                print("\t%22s: %9.2f %s" % (c['categoryName'], c['debitTotal']['amount'], c['debitTotal']['currency']))
        
        try:
            incomeData = getTransactionsSummary(acc['id'], 'INCOME')['transactionSummary']
        except:
            print("Income: None")
        else:
            print("Income:")
            incomeByCategory = incomeData[0]['categorySummary']
            for c in incomeByCategory:
                print("\t%22s: %9.2f %s" % (c['categoryName'], c['creditTotal']['amount'], c['creditTotal']['currency']))
        print(45 * "=")
...
#             try:
#                 sample_json_take_2 = getTransactionsSummary(accountId,from_year,to_year,from_month,to_month,from_day,to_day,"EXPENSE")
...
#             sample_json_take_3 = getTransactionsSummary(accountId,from_year,to_year,from_month,to_month,from_day,to_day,"INCOME")
